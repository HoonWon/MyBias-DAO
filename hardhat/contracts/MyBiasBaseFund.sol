...
// File @openzeppelin/contracts/utils/Timers.sol@v4.4.0
...
// File @openzeppelin/contracts/governance/IGovernor.sol@v4.4.0
...
// File @openzeppelin/contracts/governance/Governor.sol@v4.4.0
...
// File @openzeppelin/contracts/governance/extensions/GovernorSettings.sol@v4.4.0
...
// File @openzeppelin/contracts/governance/extensions/GovernorCountingSimple.sol@v4.4.0
...
// File @openzeppelin/contracts/governance/extensions/GovernorVotes.sol@v4.4.0
...
abstract contract GovernorVotes is Governor {
    IMyBiasBaseGovernanceToken public token;
...
// File @openzeppelin/contracts/governance/extensions/GovernorVotesQuorumFraction.sol@v4.4.0
...
// File contracts/MyBiasBaseFund.sol

contract MyBiasBaseFund is
    Governor,
    GovernorSettings,
    ...
    Ownable
{
    ...
    function init(
        ...
        strategies.push(initialStrategy);
    ...
    function _sendMatic(address payable to, uint256 amount) internal {
        uint256 maticBalance = address(this).balance;

        (bool sent, ) = to.call{value: maticBalance}("");

        require(sent, "Failed to send Ether");

        emit SendMatic(to, amount);
    }

    function sendMatic(address payable to, uint256 amount)
        external
        onlyGovernance
    {
        _sendMatic(to, amount);
    }

    function sendMaticToStrategy(uint256 id, uint256 amount)
        external
        onlyOwner
    {
        require(id < strategies.length);

        _sendMatic(strategies[id], amount);
    }

    function addStrategy(address payable strategy) external onlyGovernance {
